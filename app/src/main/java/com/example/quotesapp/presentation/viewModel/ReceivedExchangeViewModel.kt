...
@HiltViewModel
class ReceivedExchangeViewModel @Inject constructor(
    private val dataRepository: DataRepository
) : ViewModel() {
    private val _postLiveData: MutableLiveData<ExchangeFileResponse> = MutableLiveData()
    val postLiveData get() = _postLiveData

    fun getReceivedItems(userType:String,token:String){
        viewModelScope.launch {
            dataRepository.receivedExchangeItem(userType, token).catch { e->
