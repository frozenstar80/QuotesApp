...
class RecievedExchangeResponseAdapter(
    val data: List<ExchangeFileData> = arrayListOf(),
    val context: Context,
    private val searchUserItemClickListener: HomeItemClickListener,
    val  fragment:Fragment
) :
    RecyclerView.Adapter<RecievedExchangeResponseAdapter.SearchUsersViewHolder>() {
        ...
        with(holder) {
            when (holder.itemViewType) {
                video->{
                    ...
                    itemBinding.txtDesc.text = context.getString(R.string.shared_a) + result.type?.substring(0, 1)
                        ?.uppercase(
                            Locale.ROOT
                        ) + result.type?.substring(1) + context.getString(R.string.with_you)
                ...
                image ->{
                    ...
                    itemBinding.imgPhoto.setOnClickListener {
                        searchUserItemClickListener.clicked(result.item.toString(),result.type?.substring(0, 1)
                        ?.uppercase(
                            Locale.ROOT
                        ) + result.type?.substring(1).toString())
                    }
                }
                document->{
                    ...
                    itemBinding.txtDesc.text = context.getString(R.string.shared_a) + result.type + context.getString(R.string.with_you)
