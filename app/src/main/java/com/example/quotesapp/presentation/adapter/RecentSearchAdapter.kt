...
class RecentSearchAdapter(
    ...
    override fun onBindViewHolder(holder: SearchUsersViewHolder, position: Int) {
        ...
        with(holder) {
            ...
            itemBinding.userFollowing.text = "${if (result.followers.isNotEmpty()) result.followers.get(0) else 0} ${context.getString(R.string.followers)} • ${if (result.following.isNotEmpty()) result.following.get(0) else 0} ${context.getString(R.string.following)}"
            if (result.photos.isNotEmpty()) GlideLoader(context).loadUserPicture("${Constants.IMAGE_BASE_URL}${result.photos.get(0)}",itemBinding.shapeableImageView)
            }
